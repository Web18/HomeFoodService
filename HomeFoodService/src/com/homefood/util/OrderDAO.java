...
import com.homefood.beans.DeliveryBean;
import com.homefood.beans.KitchenBean;
import com.homefood.beans.OrderBean;
...
public class OrderDAO {
	...
	public static String createOrder(String orderTotal, String subTotal, String taxes, String deliveryCharge, String discount, String payment_method, String deliveryAddressId, String customerId){
		...
		try {  
			String query = ""
					...
					+ "discount, payment_method, delivery_address_id , customer_id , stage_id "
					+ ") VALUES "
					+ "( NOW(), NULL, "+orderTotal+", "+subTotal+", "+taxes+", "+deliveryCharge+", "+discount+", '"+payment_method+"', "+deliveryAddressId+", "+customerId+",1 )";
	...
	public static ArrayList<OrderBean> getOrders(String customer_id){
		...
		try {  
			conn = new DataManager().getConnection();
			   pst = conn.prepareStatement(""
	            		+ "SELECT date_in, total_amount, stage.`name` as stage "
	            		+ "FROM `order` INNER JOIN stage ON stage.id = `order`.stage_id "
            ...
            while(rs.next()){
            	OrderBean order = new OrderBean();
            	order.setDate_in(rs.getString("date_in"));
            	order.setTotal_amount(rs.getString("total_amount"));
            	order.setStage(rs.getString("stage"));
            	orders.add(order);            
            }
	...
	public static ArrayList<OrderBean>getAdminOrders(){
		ArrayList<OrderBean> orders = new ArrayList<>();
		ResultSet rs = null;
		...
		try {  
			conn = new DataManager().getConnection();
			   pst = conn.prepareStatement(""
	            		+ "SELECT `order`.id, date_in, total_amount, stage.`name` as stage "
	            		+ "FROM `order` INNER JOIN stage ON stage.id = `order`.stage_id ;");
