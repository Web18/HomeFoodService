CREATE DATABASE  IF NOT EXISTS `home_food_service` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `home_food_service`;

-- Host: 127.0.0.1    Database: home_food_service
...
--
-- Temporary table structure for view `view_fr`
--

DROP TABLE IF EXISTS `view_fr`;
/*!50001 DROP VIEW IF EXISTS `view_fr`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `view_fr` (
  `id` tinyint NOT NULL,
  `name` tinyint NOT NULL,
  `price` tinyint NOT NULL,
  `description` tinyint NOT NULL,
...
/*!50001 VIEW `view_en` AS select `item`.`id` AS `id`,`item`.`image` AS `image`,`item`.`name` AS `name`,`item`.`price` AS `price`,`item`.`description` AS `description`,`item`.`ingredients` AS `ingredients`,`size`.`name` AS `size`,`category`.`name` AS `category` from ((`item` join `size` on((`item`.`size_id` = `size`.`id`))) join `category` on((`item`.`category_id` = `category`.`id`))) order by `item`.`id` */;
...
/*!50001 DROP TABLE IF EXISTS `view_fr`*/;
/*!50001 DROP VIEW IF EXISTS `view_fr`*/;
/*!50001 SET @saved_cs_client          = @@character_set_client */;
/*!50001 SET @saved_cs_results         = @@character_set_results */;
/*!50001 SET @saved_col_connection     = @@collation_connection */;
